...
public class ServiceClientBundleTest extends AbstractServiceClientBundleTest {
    ...
    @Test
    public void getServiceInfo_SupportedTranslation_ShouldBeConstant()
            throws ServiceException {
        Set<String> expectedTargetsEn = new HashSet<>(Arrays.asList("de", "es",
                "fr", "it", "ja", "ko", "pt-BR", "zh-Hans", "zh-Hant"));
        Map<String, Set<String>> expectedMap = Collections.singletonMap("en",
                expectedTargetsEn);

        ServiceInfo serviceInfo = client.getServiceInfo();
        Map<String, Set<String>> supported = serviceInfo
                .getSupportedTranslation();
        assertEquals(
                "Supported translation should be from English to 9 fixed languages",
    ...
    @Test
    public void getBundleIds_InitialState_ShouldNotContainTestBundles()
            throws ServiceException {
        Set<String> bundleIds = client.getBundleIds();
        for (String bundleId : bundleIds) {
            assertFalse(
                    "Bundle ID list should not contain a bundle starts with "
                            + BUNDLE_PREFIX,
    ...
    @Test
    public void getBundleIds_AfterCreate_ShouldContainTestBundles()
            throws ServiceException {
    ...
    public void createBundle_NonExisting_ShouldPass() throws ServiceException {
        ...
        assertEquals("source language should be " + source, source,
                bundleData.getSourceLanguage());
        assertEquals("target langauges should be " + targets, targets,
                bundleData.getTargetLanguages());
    }

    @Test
    public void createBundle_ValidNonMTLanguage_ShouldPass()
            throws ServiceException {
        final String bundleId = testBundleId("bundle1");
        final String source = "uz";
        final Set<String> targets = new HashSet<>(
                Arrays.asList("hi", "kok", "sr-Latn"));
    ...
    @Test
    public void createBundle_EmptySourceLanguage_ShouldFail()
            throws ServiceException {
        expectedException.expect(ServiceException.class);
    ...
    @Test
    public void createBundle_BadSourceLanguage_ShouldFail()
            throws ServiceException {
        expectedException.expect(ServiceException.class);
    ...
    @Test
    public void createBundle_BadTargetLanguage_ShouldFail()
            throws ServiceException {
        expectedException.expect(ServiceException.class);
        createBundleWithLanguages(testBundleId("bundle1"), "en", "de",
    ...
    public void getBundleInfo_UpdatedBy_ShouldMatchThisUser()
        ...
        assertEquals("updatedBy field should be the test user",
                getUpdatedByValue(), bundleData.getUpdatedBy());
    }

    @Test
    public void getBundleInfo_UpdatedAt_ShouldBeRecent()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        createBundleWithLanguages(bundleId, "en");
        BundleData bundleData = client.getBundleInfo(bundleId);
    ...
    @Test
    public void getBundleInfo_Default_ShouldNotBeReadOnly()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        createBundleWithLanguages(bundleId, "en");
        BundleData bundleData = client.getBundleInfo(bundleId);
    ...
    //
    @Test
    public void getBundleMetrics_NonEmpty_MetricsEntryCountShouldMatch()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
    ...
    public void getBundleMetrics_Empty_MetricsShouldBeEmpty()
        ...
        BundleMetrics bundleMetrics = client.getBundleMetrics(bundleId);
        Map<String, String> strings = client.getResourceStrings(bundleId,
                source, false);
        checkBundleMetrics(bundleMetrics, source, targets, strings.size());
    }

    @Test
    public void getBundleMetrics_NonExisting_ShouldFail()
            throws ServiceException {
        expectedException.expect(ServiceException.class);
    ...
    @Test
    public void updateBundle_SetTargetLangs_ShouldOverwrite()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        createBundleWithLanguages(bundleId, "en", "fr", "zh-Hans");

        Set<String> newTargets = new HashSet<>(
                Arrays.asList("zh-Hans", "zh-Hant"));
        ...
        BundleData bundleData = client.getBundleInfo(bundleId);
        assertEquals("target languages should be updated", newTargets,
                bundleData.getTargetLanguages());
    }

    @Test
    public void updateBundle_AddBadTargetLang_ShouldFail()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        ...
        assertEquals("target languages should not be updated",
                new HashSet<>(Arrays.asList(origTargets)),
                bundleData.getTargetLanguages());
    }

    @Test
    public void updateBundle_Metadata_ShouldAddModifyDelete()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        createBundleWithLanguages(bundleId, "en");
        ...
        client.updateBundle(bundleId, changes);
        BundleData bundleData = client.getBundleInfo(bundleId);
        assertEquals("metadata should be added", metadata,
    ...
    @Test
    public void updateBundle_Notes_ShouldUpdate() throws ServiceException {
        String bundleId = testBundleId("bundle1");
        List<String> notes0 = Arrays.asList("Initial note");
        createBundle(bundleId, "en", Collections.singleton("fr"), notes0);

        BundleData bundleData = client.getBundleInfo(bundleId);
        assertEquals("notes should be saved", notes0, bundleData.getNotes());

        // Update bundle notes
        List<String> notes1 = Arrays.asList("New note #1", "New note #2");
        BundleDataChangeSet changes = new BundleDataChangeSet();
        changes.setNotes(notes1);
        client.updateBundle(bundleId, changes);

        bundleData = client.getBundleInfo(bundleId);
        assertEquals("notes should be updated", notes1, bundleData.getNotes());

        // Delete bundle notes
        changes.setNotes(Collections.<String> emptyList());
        client.updateBundle(bundleId, changes);

        bundleData = client.getBundleInfo(bundleId);
        assertNull("notes should be deleted", bundleData.getNotes());
    }
    ...
    public void deleteBundle_Existing_ShouldDelete() throws ServiceException {
        Set<String> bundleIds = client.getBundleIds();
        String bundleId = testBundleId("bundle1");
        assertFalse(bundleId + " should not be yet there",
                bundleIds.contains(bundleId));

        createBundleWithLanguages(bundleId, "en");
        bundleIds = client.getBundleIds();
        assertTrue(bundleId + " should be already created",
                bundleIds.contains(bundleId));

        // Now delete it
        client.deleteBundle(bundleId);
        bundleIds = client.getBundleIds();
        assertFalse(bundleId + " should be deleted",
                bundleIds.contains(bundleId));
    }
    ...
    @Test
    public void getResourceStrings_SourceLang_ShouldMatch()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
        ...
        Map<String, String> srcStrings1 = client.getResourceStrings(bundleId,
                source, false);
        assertEquals(
                "gerResourceStrings (fallback:false) for the source should match the original",
                strings, srcStrings1);

        Map<String, String> srcStrings2 = client.getResourceStrings(bundleId,
                source, true);
        assertEquals(
                "getResourceStrings (fallback:true) for the source should match the original",
                strings, srcStrings2);
    }

    @Test
    public void getResourceStrings_NonExistingLang_ShouldFail()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
        createBundleWithLanguages(bundleId, source);
    ...
    @Test
    public void getResourceStrings_Fallback_ShouldReturnSource()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
        ...
        assertEquals(
                "gerResourceStrings for " + DUMMY_FAIL_LANG
                        + " with fallback:true should match the original",
                strings, targetStrings);
    }

    @Test
    public void getResourceStrings_NoFallback_ShouldReturnEmpty()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
        createBundleWithLanguages(bundleId, source, DUMMY_FAIL_LANG);
    ...
    public void getResourceEntries_SourceLang_ShouldMatch()
        ...
        Map<String, ResourceEntryData> srcResources = client
                .getResourceEntries(bundleId, source);
        compareSourceResourceEntries(TEST_RES1, srcResources, true, true);
    }

    @Test
    public void getResourceEntries_NonExistingLang_ShouldFail()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
        createBundleWithLanguages(bundleId, source);
    ...
    @Test
    public void getResourceEntries_MTFailure_ShouldReturnNonEmpty()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
        createBundleWithLanguages(bundleId, source, DUMMY_FAIL_LANG);
        ...
        Map<String, ResourceEntryData> targetEntries = client
                .getResourceEntries(bundleId, DUMMY_FAIL_LANG);
        assertEquals(
                "gerResourceEntries for " + DUMMY_FAIL_LANG
    ...
    public void getLanguageMetrics_SourceLang_MetricsEntryCountShouldMatch()
        ...
        EnumMap<TranslationStatus, Integer> transMetrics = metrics
                .getTranslationStatusMetrics();
        ...
        checkTranslationStatusMetrics(metrics.getTranslationStatusMetrics(),
                strings.size(), EnumSet.of(TranslationStatus.SOURCE_LANGUAGE));
    ...
    public void getLanguageMetrics_TargetLang_MetricsEntryCountShouldMatch()
        ...
        checkTranslationStatusMetrics(metrics.getTranslationStatusMetrics(),
                strings.size(), EnumSet.of(TranslationStatus.TRANSLATED,
    ...
    @Test
    public void getLanguageMetrics_NonExisting_ShouldFail()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
    ...
    @Test
    public void uploadResourceStrings_NonExistinBundle_ShouldFail()
            throws ServiceException {
    ...
    @Test
    public void uploadResourceStrings_NonExistingLang_ShouldAddLang()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
        createBundleWithLanguages(bundleId, source);
        ...
        assertTrue("target languages should contain " + target,
    ...
    public void uploadResourceStrings_UploadTarget_ShouldKeepExistingKeys()
        ...
        Map<String, String> resultStrings = client.getResourceStrings(bundleId,
                target, false);
        assertEquals("set of keys should not be changed", strings.keySet(),
                resultStrings.keySet());
        assertEquals("value of " + key + " should be updated", modVal,
                resultStrings.get(key));
    }
    ...
    @Test
    public void uploadResourceEntries_NonExistingLang_ShouldAddLang()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
        createBundleWithLanguages(bundleId, source);
    ...
    @Test
    public void uploadResourceEntries_UploadSuperSet_ShouldOnlyAddNewKey()
            throws ServiceException, InterruptedException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
        String target = TRANSLIT_LANG;
        ...
        Map<String, ResourceEntryData> resultResources = client
                .getResourceEntries(bundleId, source);
        assertEquals("number of source resources should be incremented",
                numResources, resultResources.size());
        assertTrue("the new key should be added to source",
                resultResources.containsKey(newKey));
    ...
    @Test
    public void uploadResourceEntries_UploadSubSet_ShouldDeleteMissingKey()
            throws ServiceException, InterruptedException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
        String target = TRANSLIT_LANG;
        ...
        assertEquals("number of source resources should be decremented",
                numResources, resultResources.size());
        assertFalse("the new key should be added to source",
                resultResources.containsKey(removeKey));

        // target language
        resultResources = client.getResourceEntries(bundleId, target);
        assertEquals("number of target resources should be decremented",
                numResources, resultResources.size());
        assertFalse("the new key should be added to target",
                resultResources.containsKey(removeKey));
    }

    @Test
    public void uploadResourceEntries_UploadTarget_ShouldIgnoreNonExistingKey()
            throws ServiceException, InterruptedException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
        String target = TRANSLIT_LANG;
    ...
    public void uploadResourceEntries_UploadTarget_ShouldKeepExistingKeys()
        ...
        String key = TEST_RES1[0][0];
        String modVal = "XYZ";
        client.uploadResourceEntries(bundleId, target, Collections
                .singletonMap(key, new NewResourceEntryData(modVal)));
        ...
        Map<String, ResourceEntryData> resultResources = client
                .getResourceEntries(bundleId, target);
        assertEquals("set of keys should not be changed", resources.keySet(),
                resultResources.keySet());
        assertEquals("value of " + key + " should be updated", modVal,
                resultResources.get(key).getValue());
    }
    ...
    @Test
    public void updateResourceStrings_NonExistingBundle_ShouldFail()
            throws ServiceException {
        String bundleId = testBundleId("bundle0");
    ...
    @Test
    public void updateResourceStrings_NoExistingLang_ShouldFail()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        createBundleWithLanguages(bundleId, "en");
    ...
    public void updateResourceStrings_NewKeyInSource_ShouldAddKey()
        ...
        assertEquals("source lang should contain the one just added", newVal,
                srcNewVal);
        assertEquals("number of strings in source should be incremented",
                strings.size() + 1, sourceStrings.size());

        // target strings should also contain the new key
        Map<String, String> targetStrings = client.getResourceStrings(bundleId,
                target, false);
        assertTrue("target lang should contain the one just added",
                targetStrings.containsKey(newKey));
        assertEquals("number of strings in source should be incremented",
                strings.size() + 1, targetStrings.size());
    }
    ...
    public void updateResourceStrings_NewKeyInTarget_ShouldIgnore()
        ...
        Map<String, String> sourceStrings = client.getResourceStrings(bundleId,
                source, false);
        assertEquals("source lang should not be affected", strings,
                sourceStrings);

        // target strings should not be affected too.
        Map<String, String> targetStrings = client.getResourceStrings(bundleId,
                target, false);
        assertFalse("target lang should not contain the new key",
                targetStrings.containsKey("NewKey"));
        assertEquals("number of strings in target should be same",
                strings.size(), targetStrings.size());
    }
    ...
    public void updateResourceStrings_DeleteKeyInSource_ShouldDelete()
        ...
        Map<String, String> sourceStrings = client.getResourceStrings(bundleId,
                source, false);
        assertFalse("source strings should not contain " + key,
                sourceStrings.containsKey(key));

        // it should be also deleted in the target language
        Map<String, String> targetStrings = client.getResourceStrings(bundleId,
                target, false);
        assertFalse("target strings should not contain " + key,
                targetStrings.containsKey(key));
    }
    ...
    @Test
    public void updateResourceEntries_NonExistingBundle_ShouldFail()
            throws ServiceException {
        String bundleId = testBundleId("bundle0");
        String[][] strings = { { "key1", "val1" } };
        Map<String, ResourceEntryDataChangeSet> changes = toResourceEntryChangeMap(
                strings);
    ...
    @Test
    public void updateResourceEntries_NoExistingLang_ShouldFail()
            throws ServiceException {
        String bundleId = testBundleId("bundle1");
        createBundleWithLanguages(bundleId, "en");
    ...
    public void updateResourceEntries_NewKeyInSource_ShouldAddKey()
        ...
        String newKey = "NewKey";
        String newVal = "NewVal";
        Map<String, ResourceEntryDataChangeSet> changeResources = toResourceEntryChangeMap(
                new String[][] { { newKey, newVal } });
    ...
    @Test
    public void updateResourceEntry_Notes_ShouldUpdate() throws ServiceException {
        String bundleId = testBundleId("bundle1");
        String source = "en";
        createBundleWithLanguages(bundleId, source);
        String key = "key1";
        client.uploadResourceStrings(bundleId, source, Collections.singletonMap(key, "val1"));

        ResourceEntryData resEntry = client.getResourceEntry(bundleId, source, key);
        assertNull("notes should be null", resEntry.getNotes());

        // Update notes
        List<String> notes1 = Arrays.asList("My note 1", "My note 2");
        ResourceEntryDataChangeSet changeSet = new ResourceEntryDataChangeSet();
        changeSet.setNotes(notes1);
        client.updateResourceEntry(bundleId, source, key, changeSet);

        resEntry = client.getResourceEntry(bundleId, source, key);
        assertEquals("notes should be updated", notes1, resEntry.getNotes());

        // Delete notes
        changeSet.setNotes(Collections.<String> emptyList());
        client.updateResourceEntry(bundleId, source, key, changeSet);

        resEntry = client.getResourceEntry(bundleId, source, key);
        assertNull("notes should be deleted", resEntry.getNotes());
    }
    ...
    private static Map<String, NewResourceEntryData> toNewResourceEntryMap(
            String[][] resArray, boolean withSequence) {
        Map<String, NewResourceEntryData> newResEntryMap = new HashMap<>(
                resArray.length);
        int seq = 1;
        for (String[] singleResData : resArray) {
            if (singleResData[1] != null) {
                NewResourceEntryData newResEntry = new NewResourceEntryData(
                        singleResData[1]);
                if (withSequence) {
    ...
    private static Map<String, ResourceEntryDataChangeSet> toResourceEntryChangeMap(
            String[][] resArray) {
        Map<String, ResourceEntryDataChangeSet> changeResEntryMap = new HashMap<>(
                resArray.length);
        for (String[] singleResData : resArray) {
            if (singleResData[1] != null) {
    ...
    private static void compareSourceResourceEntries(String[][] expectedArray,
        ...
        for (String[] expected : expectedArray) {
            ...
            assertEquals("the result value should match the original", resVal,
                    actualEntry.getValue());

            assertEquals("the updatedBy field should match the user", expUpdBy,
                    actualEntry.getUpdatedBy());
            if (checkDateRecent) {
            ...
            if (checkSeq) {
                ...
                assertEquals("the sequenceNumber field should match", seq,
                        actualEntry.getSequenceNumber().intValue());
            }
        }
    }
    ...
    private static void checkBeingTranslated(String[][] sourceResArray,
        ...
        assertEquals("number of resources are different", sourceResArray.length,
                targetEntries.size());
        for (String[] sourceEntry : sourceResArray) {
            String key = sourceEntry[0];
            ResourceEntryData targetEntry = targetEntries.get(key);
            assertNotNull("resource entry for key " + key
                    + " is not found in the target", targetEntry);
            assertEquals(
                    "the source value for key " + key
            ...
            if (status == TranslationStatus.TRANSLATED) {
                assertNotNull(
                        "the translation status for key " + key
                ...
                fail("the translation status for key " + key
                        + " is not TRANSLATED or IN_PROGRESS");
            }
        }
    }

    private static void checkBundleMetrics(BundleMetrics metrics, String source,
            String[] targets, int numResources) {
        Map<String, EnumMap<TranslationStatus, Integer>> transMetricsByLang = metrics
                .getTranslationStatusMetricsByLanguage();

        Map<String, ReviewStatusMetrics> reviewMetricsByLang = metrics
                .getReviewStatusMetricsByLanguage();
        ...
        if (targets != null) {
            for (String target : targets) {
                ...
                assertNotNull("translation status metrics for " + target,
                        transMetrics);
                checkTranslationStatusMetrics(transMetrics, numResources,
                        EnumSet.of(TranslationStatus.TRANSLATED,
                                TranslationStatus.IN_PROGRESS));

                reviewMetrics = reviewMetricsByLang.get(target);
                assertNotNull("review status metrics for " + target,
                        reviewMetrics);
                checkReviewStatusMetrics(reviewMetrics, numResources, -1);

                partnerMetrics = partnerMetricsByLang.get(target);
                assertNotNull("partner status metrics for " + target,
                        partnerMetrics);
            }
        }
    }
     ...
     * 
     * @param metrics
     *            The input translation status metrics.
     * @param numResources
     *            The total number of resources expected.
     * @param validStatusSet
     *            If not null, check if all status values are one of status
     *            included in the set.
     */
    private static void checkTranslationStatusMetrics(
            EnumMap<TranslationStatus, Integer> metrics, int numResources,
            EnumSet<TranslationStatus> validStatusSet) {
        int actualNumResources = 0;
        for (Entry<TranslationStatus, Integer> entry : metrics.entrySet()) {
            if (validStatusSet != null) {
                assertTrue(
                        "translation status should be on of " + validStatusSet,
                        validStatusSet.contains(entry.getKey()));
            }
            actualNumResources += entry.getValue().intValue();
        }
        assertEquals("number of translation status entries", numResources,
                actualNumResources);
    }
     ...
     * 
     * @param metrics
     *            The input review status metrics.
     * @param numResources
     *            The total number of resources expected.
     * @param numReviewed
     *            If 0 or positive, check if the number actually matches the
     *            reviewed count.
     */
    private static void checkReviewStatusMetrics(ReviewStatusMetrics metrics,
        ...
        if (numReviewed >= 0) {
            assertEquals("number of reviewed", numReviewed,
                    metrics.getReviewed());
        }
    }
}
